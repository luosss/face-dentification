  ...
  <div class="register-box">
    <div style="margin-right:500px;">
      ...
      <div class="btn">
        ...
        <el-button @click="verifyIdentityFace" :disabled="isOpenCamera">人脸核身</el-button>
      </div>
    </div>
    <div>
...
import { VerifyIdentityFace} from "../utils/api";
...
function cameraing(nums) {
  ...
  const identityFaceURL = saveAsJpeg(canvas);
  if(nums === 1) {
    base64Img1.value = identityFaceURL;
  }else if(nums === 2) {
...
async function verifyIdentityFace() {
  let res = await VerifyIdentityFace({
    name:names.value,
    Identity: ids.value,
    FileBase64: base64Img1.value.substr(23),
  });
  if (res.success == true) {
    ElMessage({
      message: "实时人脸检测通过"+ res.error,
      type: "success",
    });
  } else {
    ElMessage.error("实时人脸检测失败," + res.error);
  }
}
