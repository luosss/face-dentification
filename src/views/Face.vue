<template>
  <div>
    ...
    <el-button @click="faceClick">拍照</el-button>
...
const compareFace = async (dataURL) => {
...
const detectLivingFace = async (dataURL) => {
  let res = await DetectLivingFace({
    FileBase64: dataURL.substr(22),
  });
  if (res.success == true) {
    ElMessage({
      message: "人脸检测通过," + res.error,
      type: "success",
    });
  } else {
    ElMessage.error(res.error);
...
const verifyIdentityFace = async (dataURL) => {
  let res = await VerifyIdentityFace({
    Identity: "44152319950517757X",
    FileBase64: dataURL.substr(23),
    name: "罗加宝",
  });
  if (res.success == true) {
    ElMessage({
...
const monitorExamination = async (dataURL) => {
...
const monitorExaminationByChatTool = async (dataURL) => {
